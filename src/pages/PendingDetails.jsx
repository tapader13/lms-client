...
import style from './pendingdetails.module.css';
...
function PendingDetails() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:4000/auth/detailsinfobookuser/${id}`)
      .then((res) => {
        if (res.data.status) {
  ...
  const handleReject = () => {
    axios
      .put(`http://localhost:4000/auth/rejectbookadmin/${id}`)
      .then((res) => {
        if (res.data.status) {
          toast('book request rejected!', { position: 'top-left' });
          navigate('/dashbord/issupending');
        } else {
  ...
  const handleAccept = () => {
    axios
      .put(`http://localhost:4000/auth/acceptbookadmin/${id}`)
      .then((res) => {
        if (res.data.status) {
          toast('book request accepted!', { position: 'top-left' });
          navigate('/dashbord/issupending');
        } else {
